...
| `ADMIN_API_KEY` | (未設定) | 運営お知らせ投稿(`POST /api/announcements` の `x-admin-key`)用。未設定だと投稿は 503 |
| `COURSE_DATA_YEARS` | (未設定) | 講義データが存在する年度(カンマ区切り, 例 `2026,2027`)。`/api/courses/years` が返す。未設定なら現年度のみ |
...
| GET `/api/courses/years` | `{ years: number[] }`(講義データが存在する年度。`COURSE_DATA_YEARS` 未設定なら現年度) |
...
| GET `/api/announcements` | `{ announcements: Announcement[] }`(運営お知らせ、新しい順 最大50件) |
| POST `/api/announcements` | ヘッダ `x-admin-key` 必須。body: `{ title, body }` → 保存 + 全 push 購読者へ配信、`{ id }`。キー未設定は 503 / 不一致は 401 |
...
通知対象は全件(学部・大学院を問わない)。新規分を以下の**2チャンネルに分けて**配信する:
