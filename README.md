...
| `KIT_USER_ID` | (未設定) | 休講取得用の学籍番号。設定すると KIT ポータルを直接スクレイプ |
| `KIT_PASSWORD` | (未設定) | `KIT_USER_ID` のパスワード(両方揃って初めて有効) |
...
`KIT_USER_ID` / `KIT_PASSWORD` の両方が設定されている場合、KIT の SSL-VPN
(`KIT_VPN_HOST`)のプロキシ経由で Shibboleth ログインし、ポータル
(`portal.student.kit.ac.jp`)の `c=lecture_cancellation`(休講通知)と
`c=lecture_information`(授業関連連絡)の表を直接スクレイプします(`src/lib/kitPortal.ts`)。
未設定、または取得に失敗した場合は `https://ebii.net/cancel.json` にフォールバックします。
...
| GET `/api/cancellations` | `CancellationFeed`。KIT ポータルを直接スクレイプ(認証情報未設定/失敗時は `https://ebii.net/cancel.json`)し正規化して中継(10分メモリ+DBキャッシュ。取得失敗時はキャッシュ → 空 feed) |
...
node-cron で 1 分毎(`* * * * *`)に休講フィード(ポータル直取得 or cancel.json)を取得し、前回スナップショット
